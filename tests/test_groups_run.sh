...
sudo sed -i "s/concurrency=5/concurrency=1/" /etc/mcv/mcv.conf
sudo mcvconsoler --run custom quick >> /tmp/cli_output.log
sudo echo 'quick ' $? >> /tmp/test_result.log
sudo mcvconsoler --run custom workload >> /tmp/cli_output.log
sudo echo 'workload ' $? >> /tmp/test_result.log
sudo mcvconsoler --run custom functional >> /tmp/cli_output.log
sudo echo 'functional ' $? >> /tmp/test_result.log
sudo mcvconsoler --run custom smoke >> /tmp/cli_output.log
sudo echo 'smoke ' $? >> /tmp/test_result.log
sudo mcvconsoler --run custom scale >> /tmp/cli_output.log
sudo echo 'scale ' $? >> /tmp/test_result.log
